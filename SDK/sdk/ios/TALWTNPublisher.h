...
/** {zh}
 * @type api
 * @brief TALWTNPublisher Class
 */
/** {en}
 * @type api
 * @brief TALWTNPublisher Class
 */
TALWTN_IOS_EXPORT @interface TALWTNPublisher : NSObject
/** {zh}
 * @type api
 * @brief 发布音视频流
 ...
 *        + < 0：方法调用失败  <br>
 * @notes <br>
 *        + 对于一个 TALWTNPublisher 实例，仅能同时发布一路音视频流。重复调用此接口会失败。<br>
 *        + 无论是否开启音视频采集，你都可以调用此接口开启发布。<br>
 *        + 调用此接口后，会收到 onPublishStateChanged:publishState:reason:{@link #TALWTNEngineDelegate#onPublishStateChanged:publishState:reason:}。<br>
 *        + 调用 stopPublish{@link #TALWTNPublisher#stopPublish} 取消发布。
 */
/** {en}
 * @type api
 ...
 *         + < 0: Failure <br>
 * @notes  <br>
 *        + You can only publish one media stream with one TALWTNPublisher instance. Calling this API during publishing will fail. <br>
 *        + You can call this API to start publishing media streams with or without media capture. <br>
 *        + After calling this API, you will receive onPublishStateChanged:publishState:reason:{@link #TALWTNEngineDelegate#onPublishStateChanged:publishState:reason:}. <br>
 *        + Call stopPublish{@link #TALWTNPublisher#stopPublish} to stop publishing the media stream.
 */
 ...
 *        + < 0：方法调用失败
 * @notes <br>
 *        + 调用 startPublish:{@link #TALWTNPublisher#startPublish:} 发布音视频流。<br>
 *        + 调用此接口后，会收到 onPublishStateChanged:publishState:reason:{@link #TALWTNEngineDelegate#onPublishStateChanged:publishState:reason:}。
 */
/** {en}
 * @type api
 ...
 *         + < 0: Failure <br>
 * @notes  <br>
 *         + Call startPublish:{@link #TALWTNPublisher#startPublish:} to publish the media stream. <br>
 *         + After calling this API, you will receive onPublishStateChanged:publishState:reason:{@link #TALWTNEngineDelegate#onPublishStateChanged:publishState:reason:}.
 */
- (int)stopPublish;
/** {zh}
 * @type api
 ...
 *        + 必须在调用 startVideoCapture{@link #TALWTNPublisher#startVideoCapture} 前调用此接口。
 */
/** {en}
 * @type api
 ...
 *       + Bind the view before calling startVideoCapture{@link #TALWTNPublisher#startVideoCapture}. <br>
 */
- (void)setLocalView:(UIView*)view;
/** {zh}
* @type api
* @brief 设置视频采集参数
* @param video_config 视频采集参数。参看 TALWTNVideoConfig{@link #TALWTNVideoConfig}。
* @return  <br>
*        + 0: 成功  <br>
*        + < 0: 失败  <br>
* @notes  <br>
*     + 必须在调用 startVideoCapture{@link #TALWTNPublisher#startVideoCapture} 前调用此接口。<br>
*/
/** {en}
* @type api
* @brief Set the video capture parameters.
* @param video_config See TALWTNVideoConfig{@link #TALWTNVideoConfig}.
* @return   <br>
*         + 0: Success; <br>
*         + < 0: Failure; <br>
* @notes   <br>
*         + You must call this API before calling startVideoCapture{@link #TALWTNPublisher#startVideoCapture}. <br>
*/
 ...
 * @notes  <br>
 *       + 发布前后都可调用。 <br>
 *       + 调用 stopVideoCapture{@link #TALWTNPublisher#stopVideoCapture} 可以停止摄像头采集。 <br>
 *       + 采集使用的摄像头由 switchCamera{@link #TALWTNPublisher#switchCamera} 指定。
 */
/** {en}
 * @type api
 ...
 *        + Call stopVideoCapture{@link #TALWTNPublisher#stopVideoCapture} to stop video capture. <br>
 *        + The camera used for video capture is set by switchCamera{@link #TALWTNPublisher#switchCamera}.
 */
 ...
 *        + -1: 失败。 <br>
 * @notes  <br>
 *       + 调用 startVideoCapture{@link #TALWTNPublisher#startVideoCapture} 可以开启摄像头采集。  <br>
 ...
 *         + -1: Failure. The view is empty. <br>
 * @notes  <br>
 *        + Call startVideoCapture{@link #TALWTNPublisher#startVideoCapture} to enable video capture. <br>
 ...
 *        + < 0：方法调用失败  <br>
 * @notes  <br>
 *       + 调用 stopAudioCapture{@link #TALWTNPublisher#stopAudioCapture} 可以关闭音频采集设备。 <br>
 ...
 *         + < 0: Failure <br>
 * @notes   <br>
 *        + Call stopAudioCapture{@link #TALWTNPublisher#stopAudioCapture} to stop audio capture. <br>
 ...
 *        + < 0：方法调用失败  <br>
 * @notes  <br>
 *       + 调用 startAudioCapture{@link #TALWTNPublisher#startAudioCapture} 可以开启麦克风采集。  <br>
 ...
 *         + < 0: Failure <br>
 * @notes   <br>
 *        + Call startAudioCapture{@link #TALWTNPublisher#startAudioCapture} to enable audio capture. <br>
 ...
 * @type api
 * @brief 推送外部编码帧。
 * @param encoded_frame 外部编码帧。参看 TALWTNEncodedVideoFrame{@link #TALWTNEncodedVideoFrame}。在推送过程中，帧的分辨率应保持一致。
 * @notes <br>
 *      + 不支持编码反馈和请求关键帧反馈。<br>
 *      + 根据实际帧率，周期推送外部编码帧。
 * @return <br>
 ...
 * @param encoded_frame Encoded frame. See TALWTNEncodedVideoFrame{@link #TALWTNEncodedVideoFrame}. During the process, the resolution of the frames must remain the same.
 * @notes <br>
 ...
 * @type api
 * @brief 推送外部视频帧。
 * @param video_frame 视频帧。参看 TALWTNVideoFrame{@link #TALWTNVideoFrame}。在推送过程中，帧的分辨率应保持一致，帧的宽高应为 16 的倍数。
 ...
 * @param video_frame Encoded frame. See TALWTNVideoFrame{@link #TALWTNVideoFrame}. During the process, the resolution of the frames must remain the same. The frame width and the frame height must be multiples of 16.
 ...
 * @type api
 * @brief 推送外部音频帧
 * @param audio_frame 音频帧。参看 TALWTNAudioFrame{@link #TALWTNAudioFrame}。在推送过程中，帧的参数应保持一致。
 ...
 * @param audio_frame See TALWTNAudioFrame{@link #TALWTNAudioFrame}. During the process, the parameters of the frames must remain the same. 
