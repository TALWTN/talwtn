...
#import <TalWTN/TALWTNPublisher.h>
...
@interface PushStreamingViewController ()<TALWTNEngineDelegate>
...
@property (nonatomic, strong) TALWTNPublisher *publisher;
...
- (void)dealloc{
    /// 销毁引擎
    [self.wtnEngine destroyPublisher:self.publisher];
    self.publisher = nil;
    self.wtnEngine = nil;
    TalWTNLog(@"PushStreamingViewController 销毁");
}
...
- (void)initPublisher{
    /// 创建引擎
    TALWTNPubExtraParam *extraParam = [TALWTNPubExtraParam new];;
    extraParam.pub_video_source = TALWTNPubVideoSourceInternalCamera;
    extraParam.pub_audio_source = TALWTNPubAudioSourceInternalRecord;
    self.wtnEngine = [[TALWTNEngine alloc] initWithDelegate:self];
    self.publisher = [self.wtnEngine createPublisher:extraParam];

    /// 设置视频发布参数
    TALWTNVideoConfig *videoConfig = [[TALWTNVideoConfig alloc]init];
    videoConfig.width = 640;
    videoConfig.height = 960;
    videoConfig.max_fps =15;
...
- (void)onPublishStateChanged:(NSString *)stream_id publishState:(TALWTNState)state reason:(TALWTNReason)reason {
    TalWTNLog(@"===========> state:%@ -- reason:%@", @(state), @(reason));
}
...
- (void)pushStreamingBtnClick:(UIButton *)sender  {
    ...
    NSString* streamID = self.pushStreamingUrl;
#ifdef TALWTN
    ...
    NSString *pushURL = [[NSString alloc] initWithFormat:@"%@/pub/%@/%@?MuteAudio=false&MuteVideo=false&Token=%@&TokenType=%@", HOST, appID, streamID, auth,@"Bearer"];
#else
    int expireTime = (int)[[NSDate dateWithTimeIntervalSinceNow:3600] timeIntervalSince1970];
    NSDictionary* data = @{
                    @"version":@"1.0",
    ...
    NSString* auth = [JWT encodePayload:data withSecret:appkey algorithm:[[JWTAlgorithmHS256 alloc] init]];
    NSString *pushURL = [[NSString alloc] initWithFormat:@"%@/push/%@/%@?Token=%@&TokenType=%@", HOST, appID, streamID, auth,@"Bearer"];
#endif
    [self.publisher startPublish:pushURL];
